...
class TestSearchUser(unittest.TestCase):
    ...
    @patch('main.validate_user_data', return_value=False)
    @patch('main.db')
    def test_add_user_invalid_data(self, mock_db, mock_validate_user_data):
        """
        Test that add_user returns False when validate_user_data fails.
        """
        user_data = {
            "user_id": "SC",
            "user_email": "sesame@uw.edu",
        ...
        # Verify that validate_user_data was called with the correct arguments
        mock_validate_user_data.assert_called_once_with(
            user_data['user_id'],
            user_data['user_name'],
...
class TestValidateLength(unittest.TestCase):
    ...
    def test_validate_length_exceeds_max(self):
        """
        Test that validate_length raises a ValueError when the length of value exceeds max_length.
        """
        value = "This is a very long string"
        max_length = 10

        with self.assertRaises(ValueError) as context:
            main.validate_length(value, max_length)

        self.assertTrue(f"exceeds maximum length of {max_length} characters" in str(context.exception))

    def test_validate_length_within_limit(self):
        """
        Test that validate_length returns the value when the length is within the limit.
        """
        value = "Short"
        max_length = 10
